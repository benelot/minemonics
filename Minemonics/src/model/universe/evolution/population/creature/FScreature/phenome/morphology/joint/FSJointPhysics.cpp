...
FSJointPhysics::FSJointPhysics() :
	mInWorld(false), mType(FSJointPhysics::UNKNOWN_JOINT), mJointPitchEnabled(
...
bool FSJointPhysics::equals(const FSJointPhysics& jointPhysics) const {
...
void FSJointPhysics::generateMotors(const Ogre::Vector3 maxForces) {
	mJointMaxForces = maxForces;

	// add pitch servo motor
	FSServoMotor* servoMotor = new FSServoMotor();
	servoMotor->initialize(FSJointPhysics::RDOF_PITCH, maxForces.x,
	...
	if (mType == SPHERICAL_JOINT) {
		...
		servoMotor->initialize(FSJointPhysics::RDOF_YAW, maxForces.y,
		...
		servoMotor = new FSServoMotor();
		servoMotor->initialize(FSJointPhysics::RDOF_ROLL, maxForces.z,
