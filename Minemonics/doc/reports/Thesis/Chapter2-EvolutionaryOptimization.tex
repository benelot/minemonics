...
%
The environment is a representation of a flat plane environment. %
%
...
%
The one type of environment, flat plane, simulates the environment and the creatures using newtonian mechanical physics. %
%
...
%
Commonly used juries of the simulator are the average velocity the individual escapes from the origin (AverageVelocityJury), the average height above ground of all limbs (AverageHeightJury) or the number of ground contacts during the evaluation (ContactsQtyJury). %
%
...
%
Additionally, the measure is scaled by the edge length of the total volume cube of the creature to achieve a scale-invariant measure as described in the section \ref{subsec:creatures}.

\paragraph{Average Height Jury} The average height jury measures the average height creature's limbs when performing. %
%
With this jury, creatures that grow very tall and then only fall over or creatures exploiting unrealistic torques in order to get a high fitness can be inhibited. %
%
This measure is scaled by the edge length of the total volume cube of the creature to achieve a scale-invariant measure as described in the section \ref{subsec:creatures}.
...
%
Therefore, the simulation culls them early, not considering them any more in further evaluation.
...
%
The influence factors in the morphogene branch are the flip and the mirror flag, responsible for two types of symmetry, that are described in the context of the embryogenesis in the section \label{subsec:embryogenesis}.
...
\caption[Possible indirectly encoded genome]{An example creature using the indirectly encoded genome (using a subset of the configuration possibilities). The number at the branch denotes the number of child limbs of the following type attached to the respective parent limb. Mirror means that the following branch has the mirror flag set, so that it will be mirrored.}
...
\caption[Directly encoded genome]{A figure of the same example creature using a directly encoded genome.}
...
The Phenomes are the fully developed versions of the genomes and are represented as a tree. %
%
...
%
The main issue to be resolved was that the parameter ranges in the Physics Engine were not completely clear and some lead to invalid definitions of one or multiple joints, resulting in an instable simulation of the individual. %
%
...
%
A joint constraint constrains two rigid-bodies by defining the center of rotation (CoR) of the joint in both limb coordinate reference frames, and then tries to keep both CoR definitions overlapping in the world reference frame under the additional constraints of joints limits and other influences.  %
%
...
%
For the constrained rigid-body phenotype exist many different constraint solvers, which either solve the constraint groups directly or in a numerical fashion using a number of iterations. %
%
...
The Featherstone Multibody Phenotype is based on a totally different concept called reduced coordinates or generalized coordinates (Figure \ref{figure:maximal-vs-reduced}). %
%
...
%
The Embryogenesis is influenced by these so called influence factors, which are propagated along the top-bottom path of the phenotype-tree. %
%
...
%
The mirror flag expresses the branch once in its original form and once a mirrored version along the primary axis of the limb the branch is attached to. %
%
...
%
The phenotype generator's base orientation is initialized to the quaternion identity, the segments depth limit, the current limb quantity is set to 0 and the repetition lists are emptied. %
%
Then the processing of a morphogene and its morphogene branches is repeated until a halting condition is reached and works as follows: the first generator is taken out of the list, then and it is checked if  the total limb limit or the limb depth limit defined by the individual is reached. %
%
If so, the phenotype is finished, else the Embryogenesis continues. %
%
Now we create the new limb according to the morphogene specifications. %
%
Then it is checked if this limb has to be appended to a existing parent limb (for the root morphogene no, for all others yes), if yes, the child limb is positioned so that their joint anchoring points on the surface of both limbs overlap, then they are connected by the joint prescribed by the morphogene branch. %
%
Finally, new generators are produced and inserted into our generator list, one for each morphogene branch of the child morphogene that were just processed. 
...
%
In the Minemonics simulator, the following parameters were chosen:
...
%
Furthermore it might be, that one of the fitness functions should be weighted higher than the others. %
%
...
%
To create new offsprings, from every possible parent a number of offsprings is generated. %
%
The number of offsprings per parent is found by diving the total number of new creatures to be created by crossover by the number of parents. %
%
...
%
Leaving a top percentage of creatures without variation (Elitism), it produces offspring by crossover based on the elite and an additional partner, then applies the mutations to the rest of the population according to list \ref{item:ReaperPerc}. %
%
