...
%
The Minemonics simulator is the experimentation platform used throughout this thesis and is an simulation platform that we developed especially for this thesis, mainly because other simulators for evolving virtual creatures are unpublished by the respective research groups, are outdated, or implement a different type of simulation than is needed to simulate the model described in this thesis. %
%
...
%
The relations between the components are shown in Fig. \ref{figure:universe-structure}. %
...
\caption[A figure of the simulator's universe]{A figure of the simulator's universe and its subcomponents.}
\label{figure:universe-structure}
\end{figure}
...
%
The hill environment was abandoned because disturbances to the creatures can more easily be induced by directly applying uniformly distributed forces to the body of the creature. %
%
...
%
Three degrees of freedom joint can easily be approximated by three one degree of freedom joints, as long as the three joints do not gimbal-lock, i.e. two of the degrees of freedom fall together on one.

\subsection{Epochs}
% rev. 2

The simulator allows epochs for planets to be defined, which model the changes of environmental factors, inducing a change of the fitness landscape on the planet. %
%
...
%
In the simulator, an epoch is represented by a set of juries (fitness functions) and a transition condition, which, when met, ends the epoch and starts the next epoch. %
%
However, in the experiments of this thesis, only one single infinite epoch was used, since there was no need in the experiments to change fitness conditions.
...
%
For the purpose of the experiments, juries measuring the individual's average escape velocity with respect to the origin \mbox{(\textit{AverageVelocityJury})}, the average height above ground of all body parts \mbox{(\textit{AverageHeightJury})} or the number of ground contacts during the evaluation \mbox{(\textit{ContactsQtyJury})} were developed. %
%
...
%
The culling procedure is described in section \ref{subsubsection:Culling}. %
%
...
\paragraph{Ground Contact Quantity Jury} The ground contact quantity jury measures the number of body parts that come in contact with ground during the performance. %
%
...
One or multiple populations of creatures in the simulator can live together on the same planet. %
%
Through the evolution settings of the planet it is possible to define the kind of interaction within a populations and among different populations. %
%
Available are the settings to evaluate one creature at a time, multiple creatures of one population together, multiple creatures of multiple populations or multiple whole populations at the same time within one evaluation slot. %
%
...
%
This happens when creatures apply unrealistic torques to their body (a main source of this is when creatures generate high joint torques that lead to very fast body part velocity, leading to unrealistic torques that lift creatures off the ground and make them fly). %
%
...
The size measure ensures that the fitness functions involving distances measuring different sizes of creatures are scaled appropriately to make a comparison fair. %
%
By applying it as a scaling factor, it is possible to make a distance travelled independent of the creature size, meaning that a larger creature gets the same fitness for travelling a larger distance as a smaller creature for a smaller distance, if the ratios between creature size and respective travelled distance are the same for both creatures.
...
%
The classes therefore help to evolve similar, repeated, morphological elements such as body parts of different sizes or symmetrical leg pairs, which all use the same leg definition in the genotype. %
%
...
From the class definitions in a genome, a fully developed phenome consisting of morphology and control system can be transcribed. %
%
The phenome, unlike the genome, is represented as a tree, because the cycles of the graph were broken during the transcription process. %
%
...
%
Therefore, the genotype first has to be developed into a phenotype through a transcription process called the Embryogenesis.

\paragraph{Embryogenesis}
...
To ensure a stable simulation of the evolved creature bodies, two different rigid body constraint models were evaluated for their stability when used within an evolutionary process. %
%
...
%
This issue could be fixed by using the Featherstone Multibody representation and appropriate force scaling. 
...
\subsubsection{Force scaling}

To move the body parts of a creature, an appropriate joint torque has to be applied. %
%
The appropriate torque depends on the weight of both body parts \((mass_1, mass_2)\) connected by the joint. %
%
To keep the torque tuning as simple as possible, the following formula was used: %
%
\[torque(joint_n) = scaling\_factor \cdot mass_1 \cdot mass_2\] %
% 
By hand-tuning the \(scaling\_factor\) to a value where creatures could lift themselves, but could not propel themselves forward too easily, the \(scaling\_factor\) was set to \(0.7\), which gives most evolved creatures a good stability. %
%
In the case of too much torque applied, some of the creatures begin to hover, however this issue is mostly resolved by discarding creatures that hover higher than a certain threshold above ground. %

\subsubsection{Constrained Rigid-body Phenotype}
% rev. 2

The Constrained Rigid-body Phenotype model is built from rigid-body primitives and typed joint constraints. %
%
...
%
A joint constraint constrains two rigid-bodies by defining the center of rotation of the joint in both body part coordinate reference frames. %
%
To keep up the joint constraint, the simulation tries to keep both center of rotation definitions overlapping in the world reference frame under the additional constraints of joint limits and other influences.  %
%
...
%
Therefore, the Constraint Rigidbody Phenotype was abandoned and the phenotype was reimplemented using Featherstone Multibody model. %
%
...
%
In the evolution step, the following operations are performed by the reaper: %
%
Culling a percentage of the worst performing creatures according to the total fitness ranking, variation of the remaining creatures with different variance operators according to the respective percentages and sowing of new creatures partly from crossover and random generation. %
%
In the Minemonics simulator, the creatures are sorted according to their fitness, then \unit[30]{\%} are culled. %
%
The remaining creatures are equally distributed among the variation operations Gene specification mutation, Gene split mutation, Gene purge mutation, Branch specification mutation, Grow stub gene mutation and graft feature mutation, the topmost equal share, being the fittest creatures, is left untouched following an elitism strategy. %
%
Then the missing \unit[30]{\%} are replaced with equal numbers of creatures generated by crossover and randomized instantiation.
...
%
Then, for each creature $c_n$ for $m$ fitness functions, the scores $r_i$ multiplied by the respective fitness function weights $w_i$ are summed:
...
%
Having found one of the parent, the other parent is chosen as follows: With each \(\unit[50]{\%}\) chance, the parent is either chosen as one from a tournament of 10, where the best creature of that tournament is chosen as the second partner or the second parent is chosen at random from the population. %
%
The tournament entails taking 10 creatures from the population such that the index of the best performing creature of the population has the highest probability to be chosen and the index of the worst performing creature has the lowest chance of being chosen. %
%
The probabilities for the indices come from a normal distribution with \(\mu\) at index 0 and the two \(\sigma\) distance at the last index. %
%

The actual crossover process is very similar to the commonly known one from biology. %
%
...
%
The population is partitioned into the same number of equally sized partitions as there are mutations including grafting. %
%
Based on the currently implemented mutations, this means that 6 equally size partitions are produced. %
%
To each of these partitions, one mutation is applied, the order being the same as they are presented here. %
%
The reason for the order is that the worse a creature performs, the stronger it is mutated. %
%
The probabilities were chosen by hand based on their success as a trade-off between a too low and a too high effective mutation rate. %
...
%
Each morphogene, with probability $0.2$, is reinitialized with random values. %
%
...
%
Each morphogene, with probability $0.2$, is split along the X, Y or Z axis into two morphogenes occupying roughly the same total volume as the original body part. %
%
...
%
Each morphogene, with probability $0.6$, $x$ new branches are added to the morphogene, where $x \in [0,2]$. %
%
...
%
Furthermore, the evolution discards creatures that produce high joint velocities above \(\unit[60]{Hz}\), which is usually an indication that the creature might exploit unrealistic physical forces. %
%
