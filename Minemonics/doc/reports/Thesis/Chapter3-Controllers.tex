...
The Chaotic controller that will be used control a single DoF in a joint of a creature is driven internally by the Chua circuit examined above. %
%
The chua circuit's parameters will be kept constant to keep the parameter search for evolution small, because there exist many parameter conditions that do not lead to a chaotic ground state. %
%
Instead, the initial conditions will be chosen randomly within the bounding box \((x,y,z) \in ([-2,2],[-2,2],[-2,2])\) and the integration step will be chosen randomly within \(\text{\textit{integration step}} \in [0.001,10]\). %
%
The real integration step however will always be kept constant at \(\text{\textit{integration base}} = 0.0005\). %
%
The integration step is then performed by calculating smaller sub-steps of size \textit{integration base} until a small, residual integration step is left to calculate. %
%
...
%
However in the creature's controller, The simple limiter is not directly applied as in the experiment above, but it will be tested if the controller output can be limited by evolution on the basis of the joint, so that periodic motion can arise. %
%
If this does not prove to be successful, a feedback to the chaotic system has to be implemented which limits it according to the current state of the controlled joint. %
