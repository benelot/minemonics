...
In the first experimental setting, the sinusoidal controller as described in section \ref{sec:sinusoidal-controllers} was used to evolve creatures. %
%
A population of 100 creatures was set up with an average velocity fitness function and a minimal height fitness function. %
%
The latter was chosen because the simulator had issues in the physical model setup that led to creatures with the ability to fly due to imaginary forces (as described in section \ref{subsec:populations}). %
%
...
%
In crossover (as described in section \ref{subsec:crossover}), subsections of the two participating creature genotypes are taken and combined into one new genotype. %
%
...
The major drawback of the sinusoidal oscillator controller is that it does not show any adaption to the environment during the evaluation. %
%
...
The experiments were run on the chaotic controller as described in section \ref{subsec:chua-circuit}. %
%
...
\begin{figure}[H]
%\hspace*{-4.2in}
\input{Pictures/controllers/indirect-limit-controller-joint-complex.tikz}
...
%
One can now observe the former influence of the x dimension limiter when it is removed from the system. In this case, the initial conditions were set to \([0,0,0]\) to observe the complete trajectory.
...
%
In order to observe examples of creatures that locomote using chaotic controllers, three simulators, each running a separate evolutionary setting, were started. %
%
...
%
The experiment using the direct limiter controller chaotic controller sounded much more promising, since the controller stayed in a quasi-periodic motion for all the time for all the model leg setups. %
%
...
The simple limiters of the environment apparently stabilize a certain periodic orbit, but as the gait progresses, the limiters change and therefore the controller behavior changes. %
%
The controller therefore satisfies the idea of being adaptive, however it stayed in question if multiple controllers could be combined with a co-evolved morphology so that a satisfactory locomotion pattern would arise. %
%
The fitness function of average velocity leaves it up to the individual, how the movement would be achieved. %
%
...
%
In the following paragraphs, three of these evolved creatures are examined in detail. %
%
It is important to note that all creatures use the same type of chaotic controller, namely the directly limited version. %
%
The only parameters that change among the creatures are the initial conditions of the underlying Chua circuit and the control signal speed. %
%
The input to the controller obviously is different from joint to joint as well, but is intended to be different, as it is influenced through the simple limiters. %
...
%
The quasiperiodicity might be explained by the disturbance generated by the different body parts moving simultaneously, therefore no single body part can follow a proper limit cycle. %
%
...
\node[inner sep=0pt] (controller-state-space) at (0,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/walker1/walker1-off-ground.png}
};

\node[align=left] at (9,0){\underline{Joint Dynamics}};
\node[inner sep=0pt] (joint-dynamics) at (9,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/walker1/walker1-off-ground-joint.png}
};
	\end{tikzpicture}
	\end{minipage}
\caption[Off ground controller dynamics of the walker]{Dynamics of a chaotic controller and joint if the walker is off ground. The joint limits make the joint movement stay periodic even in the absence of the ground limiter.}
...
\node[inner sep=0pt] (controller-state-space) at (0,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/walker1/walker1-on-ground.png}
};

\node[align=left] at (9,0){\underline{Joint Dynamics}};
\node[inner sep=0pt] (joint-dynamics) at (9,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/walker1/walker1-on-ground-joint.png}
};
	\end{tikzpicture}
	\end{minipage}
...
\node[inner sep=0pt] (controller-state-space) at (0,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/crawler1/crawler1-off-ground.png}
};

\node[align=left] at (9,0){\underline{Joint Dynamics}};
\node[inner sep=0pt] (joint-dynamics) at (9,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/crawler1/crawler1-off-ground-joint.png}
};
	\end{tikzpicture}
	\end{minipage}
...
\node[inner sep=0pt] (controller-state-space) at (0,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/crawler1/crawler1-on-ground.png}
};

\node[align=left] at (9,0){\underline{Joint Dynamics}};
\node[inner sep=0pt] (joint-dynamics) at (9,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/crawler1/crawler1-on-ground-joint.png}
};
	\end{tikzpicture}
	\end{minipage}
...
\node[inner sep=0pt] (controller-state-space) at (0,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/jumper1/jumper1-off-ground.png}
};

\node[align=left] at (9,0){\underline{Joint Dynamics}};
\node[inner sep=0pt] (joint-dynamics) at (9,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/jumper1/jumper1-off-ground-joint.png}
};
	\end{tikzpicture}
	\end{minipage}
...
\node[inner sep=0pt] (controller-state-space) at (0,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/jumper1/jumper1-on-ground.png}
};

\node[align=left] at (9,0){\underline{Joint Dynamics}};
\node[inner sep=0pt] (joint-dynamics) at (9,-3)
{
\includegraphics[width=0.45\textwidth]{results/evolved-creatures/chaotic/jumper1/jumper1-on-ground-joint.png}
};
	\end{tikzpicture}
	\end{minipage}
\caption[On ground controller dynamics of the crawler]{Dynamics of a chaotic controller and joint if the jumper is on ground. With the additional limiter of the ground, the dynamics of the controller change dramatically and a quasiperiodic orbit can be observed. The ground limit seems to have moved below 1, which stabilizes the trajectory on a period 1 limit cycle, hence making the creature locomote.}
\label{figure:crawler1-on-ground-controller-dynamics}
\end{figure}
